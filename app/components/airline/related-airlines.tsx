...
type RelatedAirlinesProps = {
  airlines: AirlineDefinition[];
  currentSlug: string;
};

export function RelatedAirlines({ airlines, currentSlug }: RelatedAirlinesProps) {
  const related = airlines.filter((a) => a.slug !== currentSlug);
  if (related.length === 0) return null;

  return (
    <section
      aria-labelledby="related-airlines-heading"
      className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8"
    >
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2
            id="related-airlines-heading"
            className="text-base font-semibold tracking-tight text-slate-900"
          >
            شركات طيران ذات صلة
          </h2>
          <p className="mt-1 text-sm text-slate-500">
            استكشف ناقلات أخرى على شبكة داربي
          </p>
        </div>
        <Link
          href="/#airlines"
        ...
        {related.map((airline) => {
          ...
          return (
            <Link
              ...
              aria-label={`صفحة ${airline.nameAr}`}
            >
              <article>
                ...
                <p className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3 text-xs">
                  <span className="flex items-center gap-1 text-slate-500">
                    ...
                    {airline.activeAlerts.toLocaleString("ar-EG")} تنبيهات
                  </span>
